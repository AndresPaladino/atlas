...
2. Identificar la fuente:
   - Si el comando trae ruta: `Read` directo sobre el archivo.
   - Si trae descripción ("estas notas pegadas"): pedir el contenido.
   - Si trae solo nombre ambiguo: pedir ruta exacta bajo `raw/`.
3. Confirmar `source_kind` (lecture / book / paper / notes) con el usuario si no es inferible del path.
...
Slug:
- Para teóricos: `teorico-<materia>-clase<NN>`.
- Para libros: `<autor-apellido>-<libro-abrev>` o `<autor>-ch<NN>` si la fuente cubre un capítulo.
...
aliases: ["<variantes>"]
source_kind: lecture | book | paper | notes
path: "raw/lectures/<materia>/clase04-tfi.pdf"   # ruta relativa al repo
...
- `seen_in_subjects:` = vacío si la materia no está clara, o el slug de materia si la fuente lo determina.
